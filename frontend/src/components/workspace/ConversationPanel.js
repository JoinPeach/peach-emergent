...
import { Mail, Send, RefreshCw, Edit3, Sparkles, Clock, Check, UserPlus } from 'lucide-react';
...
const ConversationPanel = ({ ticketDetails, onTicketUpdate }) => {
  ...
  // Auto-generate AI draft when ticket opens (only if not closed)
  useEffect(() => {
    if (ticketDetails && !hasAutoGenerated && ticketDetails.ticket.status !== 'closed') {
  ...
  const handleStatusChange = async (newStatus) => {
    try {
      ...
      // If changing from closed to open/in_progress, trigger AI draft
